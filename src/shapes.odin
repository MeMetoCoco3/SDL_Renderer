...
generate_plane_mesh :: proc(
	...
	// We get top/left and bottom/right 
	xLeft := -width * 0.5
	zFront := depth * 0.5
	...
	step_x := width / f32(segments_x)
	step_z := -depth / f32(segments_z) // Negative value
	...
	build_plane(
		...
		start_uv = {0, 1},
		normal = {0, 1, 0},
		row = {segments_x, {step_x, 0, 0}, {1 / f32(segments_x), 0}},
		col = {segments_z, {0, 0, step_z}, {0, -1 / f32(segments_z)}},
	)
...
generate_box_mesh :: proc(
	...
	num_vertices :=
		(get_plane_num_vertices(segments_x, segments_y) * 2 +
			get_plane_num_vertices(segments_x, segments_z) * 2 +// front/back
			get_plane_num_vertices(segments_z, segments_y) * 2) // top/bottom// left/right
	num_indices :=
		(get_plane_num_indices(segments_x, segments_y) * 2 +
			get_plane_num_indices(segments_x, segments_z) * 2 +// front/back
			get_plane_num_indices(segments_z, segments_y) * 2) // top/bottom// left/right
...
build_plane :: proc(
	...
	// We range over the positions by row and column and add corresponding vertex
	...
	// We build the triangles, looping over row.segments and col.segments (row.s *col.s *verticesPerTriangle * numTrianglesPerQuad)
...
/*
Cuando hablamos de segmentos hablamos de cuantas celulas tiene un grid a lo largo o ancho.

o-------o     (2 vertices per row: 1 segments 
|     / |
|   /   |
| /     |
o-------o

o---o---o     (3 vertices per row: 2 segments
| / | / |
o---o---o
| / | / |
o---o---o
...
// Tendra 6 indices si los segmentos son iguales a uno, ya que tendremos 2 triangulos, con 3 vertices cada uno.
@(private)
get_plane_num_indices :: proc(row_segments, col_segments: int) -> int {
	return 2 * 3 * row_segments * col_segments}
